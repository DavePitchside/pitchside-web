...
const SYSTEM_CORE_PAGES = ["about", "contact", "technology", "privacy", "terms", "account-deletion", "admin", "home", "blog", "index", ""];
  ...
  useEffect(() => {
    const fetchAllContent = async () => {
      try {
        ...
        const fetchedPosts = postsSnap.docs.map(doc => ({ id: doc.id, contentType: 'post', ...doc.data() }));
        const fetchedPages = pagesSnap.docs
          .map(doc => ({ id: doc.id, contentType: 'page', ...doc.data() }))
          .filter(page => {
            const slugToMatch = (page.slug || "").trim().toLowerCase();
            const idToMatch = (page.id || "").trim().toLowerCase();
            return !SYSTEM_CORE_PAGES.includes(slugToMatch) && !SYSTEM_CORE_PAGES.includes(idToMatch);
          });

        const mergedContent = [...fetchedPosts, ...fetchedPages].sort((a, b) => {
      ...
      <div className="flex flex-col w-full bg-[#050505] text-zinc-900 font-roobert selection:bg-[#CCFF00] selection:text-black min-h-screen">
        ...
        <header className="fixed top-0 left-0 w-full h-screen bg-[#050505] overflow-hidden flex flex-col items-center justify-center text-center z-0 px-6">
          ...
          >
            ...
            <h1 className="text-[13vw] md:text-[8rem] lg:text-[10rem] leading-[0.85] uppercase tracking-tighter text-white" style={{ fontFamily: 'var(--font-alpha)' }}>
            ...
            >
              <p>Insights, updates, and tactical analysis at the intersection of artificial intelligence and grassroots football.</p>
            </motion.div>
          </motion.div>
        </header>
        ...
        <div className="relative z-20 w-full bg-white rounded-t-[2.5rem] md:rounded-t-[4rem] shadow-[0_-20px_60px_rgba(0,0,0,0.6)] pt-24 md:pt-32 pb-32 px-6 md:px-12 lg:px-24 mt-[100vh] min-h-screen">
          ...
          {loading ? (
            <div className="w-full h-[40vh] flex flex-col items-center justify-center bg-white">
              <div className="w-8 h-8 rounded-full border-2 border-zinc-200 border-t-[#CCFF00] animate-spin mb-4" />
            </div>
          ) : posts.length === 0 ? (
            ...
            <div className="max-w-[1300px] mx-auto w-full">
              ...
              <div className="flex items-center justify-between border-b border-zinc-200 pb-8 mb-16">
                <h2 className="text-2xl font-bold uppercase tracking-tighter text-zinc-900" style={{ fontFamily: 'var(--font-alpha)' }}>Latest Transmissions</h2>
                      ...
                      >
                        ...
                        <div className="flex flex-col flex-1 px-1">
                          ...
                          <p className="text-zinc-500 font-light leading-relaxed mb-8 line-clamp-3 text-[15px]">
                            {post.metaDescription || "Explore the tactical depths and data-driven insights in this latest breakdown."}
                          </p>
